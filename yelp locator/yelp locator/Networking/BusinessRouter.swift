//
//  BusinessRouter.swift
//  Yelp Locator
//
//  Created by Steven Layug on 7/04/21.
//

import Foundation
import UIKit
import Moya

class BusinessRouter {
    let provider = MoyaProvider<UserRequest>()
    
    public func startBusinessListRequest(options: BusinessRequestOptions, completion: @escaping (_ users: Business?, _ error: String?) -> Void) {
        provider.request(.businessList(options: options)) { result in
            switch result {
            case .success(let response):
                do {
                    print(try response.mapJSON())
                    if let results = try? response.map(Business.self) {
                        print(results)
                        completion(results, nil)
                    }
                } catch {
                    completion(nil, "No businesses retrieved")
                    return
                }
            case .failure(let error):
                completion(nil, error.localizedDescription)
                return
            }
        }
    }
}
